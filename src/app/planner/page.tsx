          ...
          <Card key={slot}>
            ...
            <div className="grid gap-2 md:grid-cols-7">
              {dayNames.map((day, idx) => {
                ...
                return (
                  <div key={day} className="rounded-xl border border-white/10 p-2 text-sm">
                    ...
                    <div className="mt-2 flex gap-1">
                      ...
                      <Button size="sm" variant="ghost" onClick={async () => { if (entry) { await plannerRepo.remove(entry.id); } }}>x</Button>
                    </div>
                    <Input className="mt-1 h-7 text-xs" placeholder="Custom meal" value={custom} onChange={(e) => setCustom(e.target.value)} onBlur={async () => {
                      if (!custom.trim()) return;
                      await plannerRepo.save({ weekStart, dayIndex: idx, slot, customMeal: custom });
                      setCustom("");
                    }} />
                  </div>
                );
